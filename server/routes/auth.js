...
router.post('/forgot-password', passwordResetLimiter, async (req, res) => {
  try {
    ...
    // Create reset URL
    const resetUrl = `${process.env.FRONTEND_URL || 'https://proguide-xfbv.vercel.app'}/reset-password?token=${resetToken}&email=${encodeURIComponent(normalizedEmail)}`;
...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        res.redirect(
          `https://proguide-xfbv.vercel.app/auth/callback?token=${token}&user=${encodeURIComponent(
            JSON.stringify({
              id: req.user._id,
              name: req.user.name,
        ...
        res.redirect("https://proguide-xfbv.vercel.app/login?error=oauth_failed");
      }
    }
  );
} else {
  ...
  router.get("/google/callback", (req, res) =>
    res.redirect("https://proguide-xfbv.vercel.app/login?error=oauth_not_configured")
  );
}
    ...
    const emailHtml = `
      <div style="max-width: 600px; margin: auto; font-family: 'Segoe UI', sans-serif; padding: 20px; background-color: #f9f9f9; border: 1px solid #ddd; border-radius: 8px;">
        ...
        <div style="text-align: center; margin-top: 30px;">
          <a href="https://proguide-xfbv.vercel.app" style="padding: 10px 20px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 5px;">Go to Dashboard</a>
        </div>
